...
import { saveMaterialStoreSideLineOneHttp } from "../Http/MaterialStoreSideLineHttp";
const ModalAddMaterialStore = () => {
  ...
  async function saveMaterialStoreSideLineOne(pd_code) {
    await saveMaterialStoreSideLineOneHttp(pd_code);
    dispatch(fetchMaterialStoreSideLineAsync());
    document.getElementById("btn-close-add-material-store").click();
  }
  return ReactDOM.createPortal(
    <>
      <Modal id="modal-add-material-store-side-line" style={{ width: 1350 }}>
        <ModalContent>
          <ModalHeader>
                    ...
                    {productStatus.loading === false &&
                      product.map((item, index) => (
                        <tr key={index}>
                          ...
                          <td>{item.customer}</td>
                          <td>
                            <button onClick={() => saveMaterialStoreSideLineOne(item.pd_code)} className="btn bg-green waves-effect" type="button">
          ...
          <ModalFooter color="light-blue">
            <button id="btn-close-add-material-store" type="button" className="btn bg-red waves-effect" data-dismiss="modal">
...
export default ModalAddMaterialStore;
